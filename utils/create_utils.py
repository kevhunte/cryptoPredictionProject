...
dropout=0.0, loss="", optimizer="", bidirectional=False, activation=""):
    ...
    model.compile(loss=loss, metrics=[loss, "mean_squared_error"], optimizer=optimizer)
    model_name = f'{sequence_length}-{n_features}-{units}-{cell.__name__}-{n_layers}-{dropout}-{loss}-{optimizer}-{bidirectional}-{activation}'
